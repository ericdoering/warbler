...
def like_message(msg_id):
    ...
    likes = Likes.query.filter(Likes.message_id == msg_id, Likes.user_id == g.user.id).all()
    
    if len(likes) > 0:
        previous_like = likes[0]
        db.session.delete(previous_like)
        db.session.commit()
        return redirect("/")
    
    like = Likes(message_id=msg_id, user_id=g.user.id)
    db.session.add(like)
    db.session.commit()
...
# @app.route("/users/add_like/<int:msg_id>/delete", methods=["GET","POST"])
